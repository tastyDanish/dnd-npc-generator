...
class TestApp(unittest.TestCase):
    ...
    def test_npc_npc_name(self):
        ...
        actual_name_type = type(self.test_npc.name.value)

        assert actual_name_type == expected_name_type

    def test_input_vals(self):
        input_npc = NPC()
        input_npc.generate_npc(level=9, race='Human', archetype='Bulky')
        print(input_npc)
